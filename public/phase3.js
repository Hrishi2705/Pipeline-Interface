...
async function runScripts4() {
    const increment = 1;

    // Run script 26
    await fetch('/runScript10')
        .then(async () => {
            updateStepProgressBar(increment);
            // Run script 23
            await fetch('/runScript9')
                .then(async () => {
                    updateStepProgressBar(increment * 2);
                    // Run script 24
                    await fetch('/runScript5')
                        .then(async () => {
                            updateStepProgressBar(increment * 3);
                            // Run script 25
                            await fetch('/runScript7')
                                .then(() => {
                                    updateStepProgressBar(increment * 4);
                                });
                        });
                });
        })
        .catch(error => {
            ...
            // Handle the error, e.g., break the sequence or perform error recovery
        });
}
