...
// Updated checkAndUploadCSV function to handle multiple files
...
// Update the event listener for inputFile change to enable the checkAndUpload button
...
async function runScripts() {
    const scriptIds = Array.from({
        length: 8
    }, (_, i) => i + 3); // Array of script IDs from 3 to 22
        ...
        try {
            await fetch(`/runScript${scriptId}`)
                .then(async () => {
                    ...
                    updateStepProgressBar(scriptId - 2);
                });
        } catch (error) {
